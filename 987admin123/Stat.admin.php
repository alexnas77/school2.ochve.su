  ...
  function fetch()
  {
    ...
    $this->title = $this->lang->STATISTICS;
  	...
  	if(isset($_GET['category_id']))
  	{
 		...
 		### All products
 		$query = "SELECT * FROM products WHERE products.category_id='$category_id' ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$ps = $this->db->results();
 		$products=array();
 		foreach($ps as $p)
  		...
  		### Today
 		$query = "SELECT *, SUM(stat.hits) as hits FROM products LEFT JOIN stat ON stat.product_id = products.product_id WHERE date=CURDATE() AND products.category_id='$category_id' GROUP BY products.brand, products.model ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$tps = $this->db->results();
 		...
 		### Yesterday
 		$query = "SELECT *, SUM(stat.hits) as hits FROM products LEFT JOIN stat ON stat.product_id = products.product_id WHERE date=DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND products.category_id='$category_id' GROUP BY products.brand, products.model ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$yps = $this->db->results();
 		...
 		### Week
 		$query = "SELECT *, SUM(stat.hits) as hits FROM products LEFT JOIN stat ON stat.product_id = products.product_id WHERE date>=DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND date<=CURDATE() AND products.category_id='$category_id' GROUP BY products.brand, products.model ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$wps = $this->db->results();
 		$week_products = array();
 		foreach($wps as $wp)
 		...
 		### Month
 		$query = "SELECT *, SUM(stat.hits) as hits FROM products LEFT JOIN stat ON stat.product_id = products.product_id WHERE date>=DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND date<=CURDATE() AND products.category_id='$category_id' GROUP BY products.brand, products.model ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$mps = $this->db->results();
 		...
 		### Year
 		$query = "SELECT *, SUM(stat.hits) as hits FROM products LEFT JOIN stat ON stat.product_id = products.product_id WHERE date>=DATE_SUB(CURDATE(), INTERVAL 1 YEAR) AND date<=CURDATE() AND products.category_id='$category_id' GROUP BY products.brand, products.model ORDER BY products.brand, products.model";
 		$this->db->query($query);
 		$rps = $this->db->results();
 		$year_products = array();
 		foreach($rps as $rp)
