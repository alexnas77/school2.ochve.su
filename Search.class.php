...
class Search extends Widget
{
  ...
  function fetch()
  {
    $keyword = $this->param('keyword');
      ...
      $query = "SELECT products.*, products_fotos.filename as filename, categories.name as category, products.price*(100-".($this->user->discount?$this->user->discount:0).")/100*".$this->currency->rate."/currencies.rate as discount_price  FROM categories, products LEFT JOIN products_fotos ON products_fotos.product_id=products.product_id LEFT JOIN currencies ON currencies.code = products.currency_id WHERE categories.category_id = products.category_id AND products.enabled='1' $s GROUP BY products.product_id ORDER BY products.brand, products.model, products_fotos.foto_id";
